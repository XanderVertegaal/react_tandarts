...
const getRandomPerson = async () => {
  try {
    ...
    return randomPeopleData.results[0]
  } catch (error) {
    console.log(error)
  }
}
...
const getDentists = async () => {
  const results = await getRandomPerson()
  console.log(results)
  const dentists = Array(4)
    .fill(0)
    .map(() => getRandomPerson())
...
const getRandomName = () => {
  getRandomPerson()
  // const person = randomPeople[Math.floor(Math.random() * 250)];
...
export { generateRandomAppointments, getDentists }
